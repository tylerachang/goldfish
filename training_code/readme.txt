...
See the header of each script for details and required data files. We then run the training scripts generated by prepare_training_scripts.py:
...
The training scripts used in our paper are included in training_scripts. However, those specific scripts rely on the configs, tokenizers, and datasets already existing in the correct directories (i.e. all the Python scripts above have been run). Note that the config file gpt_[size]_config.json is automatically updated with the correct vocab size for each model.
